...
### La voix — Amazon Polly, puis ElevenLabs

Polly Neural sur le plan Starter (16 $ le million de caractères), ElevenLabs
sur Pro et Business. La voix off n'est pas qu'un son : elle renvoie un
...
essai gratuit et filigrane, stockage R2. **298 tests**, base de test séparée.
...
### Un écart à signaler tout de suite

Les tarifs annoncent **Amazon Polly** sur le plan Starter. Seul **ElevenLabs**
est codé. La marge de 41 % du plan Starter suppose Polly ; avec ElevenLabs
elle n'y est pas.

C'est le genre d'écart qu'une équipe découvre trois semaines trop tard.
...
**1. Faire sortir une vidéo complète.** Plans animés, puis montage. C'est le
seul jalon qui change quelque chose : tant qu'aucune vidéo ne sort, tout le
reste est de la préparation.

Un raccourci utile : HyperFrames rend en local avec un Chrome et FFmpeg.
Lambda est la cible de production, **pas** une condition pour voir une
première vidéo.

**2. Coder Amazon Polly.** La marge du plan Starter en dépend (section 5).

**3. La file de jobs.** Aujourd'hui chaque étape est un appel manuel. Sans
file, il n'y a ni reprise après échec, ni écran de suivi, ni orchestration
n8n possible.

**Ne pas commencer par n8n.** Il orchestre des étapes, et il en manque trois.
Le brancher maintenant, c'est câbler du vide.
...
```bash
pnpm test        # 298 tests, base de test séparée
pnpm typecheck
pnpm build
```
